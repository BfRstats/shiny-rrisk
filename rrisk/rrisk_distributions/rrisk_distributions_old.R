...
rrisk_rgompertz <- function(n, shape = 1, scale = 1, type = "MC")
{
  p <- rrisk_runif(n, type = type)
  rrisk_qgompertz(p = p, shape = shape, scale = scale)
}

rrisk_qgompertz <- function(p, shape = 1, scale = 1)
{
  (1/scale) * log(1 - (1/shape) * log(1 - p))
}

rrisk_dgompertz <- function(x, shape = 1, scale = 1)
{
