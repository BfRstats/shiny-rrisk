...
rriskModelClass$set("private", 
  "is_correct_syntax", function(code, check_for_reserved_words = TRUE) 
  {
    ...
    if ("error" %in% attr(parse_result, "class")) {
      ...
      error_occured <- FALSE
      error_message <- ""
      # check if forbidden tokens were used
      if ("<<-" %in% parse_result$text) {
        error_occured <- TRUE
        error_message <- paste(error_message,
                               "\nForbidden tokens were used: <<-")
      }
      # find symbol names of all symbols in code
      symbol_calls <- with(parse_result, text[token == "SYMBOL"])
      # check if reserved words, or forbidden word, were used as var name
      if (check_for_reserved_words) {
        not_allowed_symbols <- symbol_calls %in% c(private$reserved_words)
...
rriskModelClass$set("private", 
  "are_params_in_range", function(user_input, dist_info)
  {
    ...
    # some optional parameter for parametric distributions contain information
    # needed to validate other parameter of the distribution.
    #
    # obtain logical vector indicating if param is optional (TRUE), 
    # or not (FALSE)
    is_opt_param <- sapply(X   = dist_info$def, 
                           FUN = function(x) x$optional)
    # get optional parameter names
    opt_param_names <- names(is_opt_param)[is_opt_param]
    # extend user input with not set optional parameter of the distribution
    not_setted_input <- sapply(
      X   = opt_param_names[!(opt_param_names %in% names(user_input))],
      FUN = function(opt_param_name) 
      {
        param_init <- dist_info$def[[opt_param_name]]$init
    ...
    # fuse user_input and not set input to the whole input for this 
    # distribution
    extended_user_input <- c(user_input, not_setted_input)
    ...
    for (param_name in names(user_input)) {
      ...
      for (check_rule in check_rules) {
        ...
        # output is (or should be) a logical value
        eval_result <- eval(expr  = str2lang(check_rule), 
        ...
        result <- private$is_error(
          test          = isFALSE(eval_result),
          error_message = paste("Parameter", dQuote(param_name), 
                                "is out of range.\n",
                                param_name, " = ", 
                                paste(user_input[[param_name]],
                                      collapse =", "), "\n", 
                                "Rule is:", check_rule, "\n")
        )
        if (!result$is_ok) return(result)
