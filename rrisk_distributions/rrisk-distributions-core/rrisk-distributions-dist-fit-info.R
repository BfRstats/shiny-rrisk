...
dist_info_list <- list(
  ...
  "gumbel" = list(
    ...
    type         = "continuous",
    range        = c(min = -Inf, max = Inf),
    pdf_func     = "rrisk_dgumbel",
    cdf_func     = "rrisk_pgumbel",
    start_values = function(x) {
      c(mu   = mean(x),
        beta = sd(x))
    },
    lower        = c(mu = -Inf, beta = 0),
    upper        = c(mu = Inf, beta = Inf)
  ),
  "gompertz" = list(
    display_name = "Gompertz",
    type         = "continuous",
    range        = c(min = 0, max = Inf),
    pdf_func     = "rrisk_dgompertz",
    cdf_func     = "rrisk_pgompertz",
    start_values = function(x) {
      shape <- 1/sd(x)
      c(location = (shape * log(0.5))/(1 - exp(median(shape * x))),
        shape    = shape)#sd(x))
    },
    lower        = c(shape = 0, scale = 0),
    upper        = c(shape = Inf, scale = Inf)
  ),
  "pert" = list(
