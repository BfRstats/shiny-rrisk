documentation_server <- function(input, output, rrisk_model)
{
  replace_author <- reactiveVal()
  remind_author <- reactiveVal()
  ...
  observeEvent(
    ...
    handlerExpr = {
      ...
      # get author info
      result <- rrisk_model()$get_author(author_name = author_name, 
                                         silent      = TRUE)
  ...
  observeEvent(
    ...
    handlerExpr = {
      if (replace_author()) {
        result <- rrisk_model()$change_author(
  ...
  output$author_table <- DT::renderDT({
    create_author_table(rrisk_model()$get_author_list(), 
